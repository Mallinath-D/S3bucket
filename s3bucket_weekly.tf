...
resource "aws_s3_bucket" "S3-bucket-weekly" {  
  bucket              = "${random_pet.petname.id}-weekly"
  acl                 = var.acl
...
resource "aws_s3_bucket_public_access_block" "s3-weekly-Public" {

  bucket = "${aws_s3_bucket.S3-bucket-weekly.id}"
